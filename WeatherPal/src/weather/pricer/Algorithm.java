...
public class Algorithm
{
	...
	public static double point_to_line_segment(double p[], double a[], double b[])
	{
		...
		if(vector_ab_square == 0){
			vector_ab_square = (p_lat - a_lat) * (-lat_difference) + (p_lon - a_lon) * (-lon_difference) ;
		}
		...
		double height = 2*area / ab;
		 ...
		 if (r <= 0) {
	    	return Math.round(ap * 100.0)/100.0; //AP
	    } 
		else if (r >= 1) {
			return Math.round(bp * 100.0)/100.0; //BP
	    }		   
	    else {
